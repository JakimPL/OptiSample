...
def test_encode_sets_rate_depth_and_gain(make_encode_ctx) -> None:
...
def test_encode_trims_to_duration(make_encode_ctx) -> None:
...
def test_encode_render_round_trip_is_near_lossless(make_encode_ctx) -> None:
...
def test_render_transpose_octave_halves_length(make_encode_ctx) -> None:
...
def test_render_volume_scales_amplitude(make_encode_ctx) -> None:
...
def test_render_zero_duration_is_empty(make_encode_ctx) -> None:
    stored = encode(sine(440.0), SR, EncodingParams(target_rate=SR, depth_bits=16), make_encode_ctx(60))
    assert render(stored, SR, pitch=60, duration_s=0.0).size == 0


def test_render_duration_pads_and_truncates(make_encode_ctx) -> None:
...
def test_encode_loop_stores_attack_plus_loop_and_drops_the_tail(make_encode_ctx) -> None:
...
def test_render_loop_sustains_a_note_held_past_the_stored_length(make_encode_ctx) -> None:
    params = EncodingParams(target_rate=SR, depth_bits=16, loop=True)
    stored = encode(sine(440.0, dur=2.0), SR, params, make_encode_ctx(60))
...
def test_loop_falls_back_to_trim_on_non_periodic_material(make_encode_ctx) -> None:
