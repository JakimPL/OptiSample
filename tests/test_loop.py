...
def test_estimate_period_recovers_the_fundamental() -> None:
    assert _estimate_period(_sine(SR), SR) == pytest.approx(PERIOD, abs=1)


def test_detect_loop_on_a_pure_tone_is_an_integer_number_of_periods() -> None:
    loop = detect_loop(_sine(SR), SR)
...
def test_detect_loop_declines_on_noise() -> None:
...
def test_looping_a_tone_reproduces_its_continuation() -> None:
    signal = _sine(SR)
...
def test_estimate_period_rejects_short_and_degenerate_bands() -> None:
    assert _estimate_period(np.zeros(4), SR) is None  # too few frames
    assert _estimate_period(_sine(20), 100_000) is None  # pitched band collapses (high <= low)
    assert _estimate_period(np.zeros(SR), SR) is None  # silence -> no peak above the correlation floor


def test_detect_loop_shrinks_to_the_periods_that_fit_a_short_steady_region() -> None:
    ...
    loop = detect_loop(_sine(700, freq=400.0), SR)
    assert loop is not None
    assert loop.length % 20 == 0 and 3 * 20 <= loop.length < int(0.05 * SR)


def test_detect_loop_declines_when_too_few_periods_fit() -> None:
    assert detect_loop(_sine(610, freq=400.0), SR) is None  # only ~2 periods fit, below the 3-period minimum
