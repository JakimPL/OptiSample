...
def test_generate_demo_writes_notes_and_audio(synth_config: SynthConfig, tmp_path: Path) -> None:
    outputs = generate_demo(tmp_path, synth_config, sample_rate=QUICK_RATE)
...
def test_generate_demo_is_deterministic(synth_config: SynthConfig, tmp_path: Path) -> None:
    a = generate_demo(tmp_path / "a", synth_config, sample_rate=QUICK_RATE, seed=7)
    b = generate_demo(tmp_path / "b", synth_config, sample_rate=QUICK_RATE, seed=7)
