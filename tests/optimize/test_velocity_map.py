...
def test_map_matches_amplitude_ratios(velocity_config: VelocityConfig) -> None:
    # Same sine at 0.1 / 0.4 / 1.0 → loudness differs by exactly 20*log10(ratio),
    # so volume = round(64 * ratio): 6.4→6, 25.6→26, 64→64.
    loud = loudness_by_velocity([(40, sine(0.1)), (80, sine(0.4)), (115, sine(1.0))], SR)
    vmap = derive_velocity_map(loud, velocity_config)
    assert vmap.volume(115) == 64
    assert vmap.volume(80) == 26
    assert vmap.volume(40) == 6


def test_map_is_monotone_bounded_and_full_length(velocity_config: VelocityConfig) -> None:
    loud = loudness_by_velocity([(30, sine(0.2)), (70, sine(0.5)), (120, sine(1.0))], SR)
    vmap = derive_velocity_map(loud, velocity_config)
...
def test_flat_extrapolation_beyond_anchors(velocity_config: VelocityConfig) -> None:
    vmap = derive_velocity_map(loudness_by_velocity([(40, sine(0.25)), (100, sine(1.0))], SR), velocity_config)
...
def test_single_velocity_is_full_volume_everywhere(velocity_config: VelocityConfig) -> None:
    vmap = derive_velocity_map(loudness_by_velocity([(64, sine(0.5))], SR), velocity_config)
    assert set(vmap.volumes) == {64}


def test_all_silent_maps_to_zero(velocity_config: VelocityConfig) -> None:
    silence = np.zeros(int(0.6 * SR), dtype=np.float64)
    vmap = derive_velocity_map(loudness_by_velocity([(40, silence), (100, silence)], SR), velocity_config)
    assert set(vmap.volumes) == {0}
    assert all(anchor.volume == 0 for anchor in vmap.anchors)


def test_anchors_record_the_measured_volume(velocity_config: VelocityConfig) -> None:
    vmap = derive_velocity_map(loudness_by_velocity([(50, sine(0.5)), (100, sine(1.0))], SR), velocity_config)
    for anchor in vmap.anchors:
        assert anchor.volume == vmap.volume(anchor.velocity)


def test_derive_requires_measurements(velocity_config: VelocityConfig) -> None:
    with pytest.raises(ValueError):
        derive_velocity_map({}, velocity_config)


def test_volume_rejects_out_of_range_velocity(velocity_config: VelocityConfig) -> None:
    vmap = derive_velocity_map(loudness_by_velocity([(64, sine(0.5))], SR), velocity_config)
    with pytest.raises(ValueError):
        vmap.volume(128)
    with pytest.raises(ValueError):
