...
_SETTLED_DEPTH = BitDepth.SIXTEEN
...
def swept() -> tuple[EncodingParams, ...]:
    ...
    span = EncodingParams(target_rate=_SETTLED_RATE, depth=_SETTLED_DEPTH, trim_s=2.0, loop_index=UNLOOPED)
...
def test_the_sweeps_own_encodings_lead_the_set(
    ...
    widened = widened_encodings(
        swept, RankingGrid(depths=(BitDepth.SIXTEEN, BitDepth.EIGHT), rate_steps=1), ladder_sweep, sample_rate=SR
    )
...
def test_a_depth_the_grid_names_joins_the_set_at_the_settled_rate(
    ...
    widened = widened_encodings(
        swept, RankingGrid(depths=(BitDepth.EIGHT,), rate_steps=0), ladder_sweep, sample_rate=SR
    )

    added = [params for params in widened if params not in swept]
    assert {(params.target_rate, params.depth) for params in added} == {(_SETTLED_RATE, 8)}
...
def test_a_widened_depth_is_offered_at_both_settings_of_the_compressor(
    ...
    widened = widened_encodings(
        swept, RankingGrid(depths=(BitDepth.SIXTEEN, BitDepth.EIGHT), rate_steps=0), ladder_sweep, sample_rate=SR
    )

    added = {(params.depth, params.compress) for params in widened if params not in swept}
...
def test_the_rungs_read_are_the_ones_the_ladder_holds_below_the_settled_rate(
    ...
    widened = widened_encodings(
        swept, RankingGrid(depths=(BitDepth.SIXTEEN,), rate_steps=2), ladder_sweep, sample_rate=SR
    )
...
def test_a_clip_is_read_at_its_own_rate_alone_where_the_grid_steps_nowhere(
    ...
    widened = widened_encodings(
        swept, RankingGrid(depths=(BitDepth.SIXTEEN,), rate_steps=0), ladder_sweep, sample_rate=SR
    )
...
def test_a_set_names_each_encoding_once(
    ...
    widened = widened_encodings(
        swept, RankingGrid(depths=(BitDepth.SIXTEEN, BitDepth.SIXTEEN), rate_steps=1), ladder_sweep, sample_rate=SR
    )
