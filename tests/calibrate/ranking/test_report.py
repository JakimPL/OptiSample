...
def test_a_metric_calling_every_pair_the_listeners_way_confirms_all_of_them() -> None:
    judgments = [_judgment("a", 2), _judgment("b", -1), _judgment("c", 1)]

    read = metric_agreement(_readings(("a", 0.8), ("b", -0.2), ("c", 0.3)), judgments)
...
def test_a_metric_calling_every_pair_the_other_way_confirms_none_of_them() -> None:
    ...
    assert read.overall.decided == len(judgments)


def test_a_tie_is_ranked_without_being_counted_among_the_calls_a_listener_made() -> None:
    judgments = [_judgment("a", 2), _judgment("b", 0), _judgment("c", -2)]

    read = metric_agreement(_readings(("a", 0.9), ("b", 0.1), ("c", -0.9)), judgments)

    assert read.overall.decided == len(judgments) - 1
...
def test_a_metric_is_read_against_each_question_apart_from_the_whole_sheet() -> None:
    judgments = [
        _judgment("a", 2, axis=PairAxis.DEPTH),
        _judgment("b", 2, axis=PairAxis.DEPTH),
        _judgment("c", 2, axis=PairAxis.LOOP),
    ]

    read = metric_agreement(_readings(("a", 0.5), ("b", 0.5), ("c", -0.5)), judgments)
...
def test_a_question_the_sheet_never_asked_is_left_out_of_the_reading() -> None:
    read = metric_agreement(_readings(("a", 0.5)), [_judgment("a", 2, axis=PairAxis.RATE)])
...
def test_the_margins_separate_what_a_listener_called_from_what_they_heard_as_level() -> None:
    judgments = [_judgment("a", 2), _judgment("b", -2), _judgment("c", 0), _judgment("d", 0)]

    read = metric_agreement(_readings(("a", 0.8), ("b", -1.2), ("c", 0.1), ("d", 0.3)), judgments)
...
def test_a_sheet_holding_calls_alone_leaves_the_separation_unstated() -> None:
    read = metric_agreement(_readings(("a", 0.8), ("b", -1.2)), [_judgment("a", 2), _judgment("b", -2)])
...
def test_the_margins_read_a_pair_heard_as_one_recording_apart_from_the_rest() -> None:
    judgments = [_judgment("a", 2), _judgment("b", 0), _judgment("c", 0, identical=True)]

    read = metric_agreement(_readings(("a", 1.0), ("b", 0.4), ("c", 0.1)), judgments)
...
def test_a_sheet_where_every_pair_was_told_apart_leaves_the_headroom_unstated() -> None:
    read = metric_agreement(_readings(("a", 0.8), ("b", 0.2)), [_judgment("a", 2), _judgment("b", 0)])
...
def test_a_metric_reading_nothing_between_an_identical_pair_states_no_headroom() -> None:
    judgments = [_judgment("a", 2), _judgment("b", 0, identical=True)]

    read = metric_agreement(_readings(("a", 0.8), ("b", 0.0)), judgments)
...
def test_a_metric_reading_every_pair_level_states_no_separation() -> None:
    judgments = [_judgment("a", 2), _judgment("b", 0)]

    read = metric_agreement(_readings(("a", 0.0), ("b", 0.0)), judgments)
...
def test_one_answered_question_is_too_few_to_correlate() -> None:
    read = metric_agreement(_readings(("a", 0.5)), [_judgment("a", 2)])
...
def test_a_listener_answering_a_repeated_question_alike_reaches_the_ceiling() -> None:
    judgments = [
        _judgment("a", 2, question_id=7),
        _judgment("b", 2, question_id=7),
        _judgment("c", -1, question_id=9),
        _judgment("d", -1, question_id=9),
    ]
...
def test_a_repeated_question_is_read_through_the_blinding_each_occurrence_was_given() -> None:
    judgments = [
        _judgment("a", 2, question_id=7, swapped=False),
        _judgment("b", -2, question_id=7, swapped=True),
    ]
...
def test_a_listener_reversing_themselves_confirms_none_of_the_repeats() -> None:
    judgments = [_judgment("a", 2, question_id=7), _judgment("b", -2, question_id=7)]
...
def test_a_repeat_the_listener_tied_once_is_left_out_of_the_calls_it_is_read_over() -> None:
    judgments = [_judgment("a", 2, question_id=7), _judgment("b", 0, question_id=7)]
...
def test_a_set_putting_every_question_once_states_no_ceiling() -> None:
    ceiling = self_agreement([_judgment("a", 2, question_id=1), _judgment("b", 2, question_id=2)])
...
def test_a_listener_reaching_for_the_louder_side_reads_as_a_level_confound() -> None:
    judgments = [
        _judgment("a", 2, loudness_delta_lu=_LOUD_GAP),
        _judgment("b", -2, loudness_delta_lu=-_LOUD_GAP),
    ]

    level = level_confound(judgments)

    assert level.gapped == len(judgments)
...
def test_a_gap_too_small_to_hear_is_left_out_of_the_confound() -> None:
    judgments = [_judgment("a", 2, loudness_delta_lu=_QUIET_GAP), _judgment("b", -2, loudness_delta_lu=_NO_GAP)]
...
def test_a_report_ranks_every_metric_it_is_handed_over_the_same_answers() -> None:
    judgments: Sequence[Judgment] = [_judgment("a", 2), _judgment("b", -2)]
    readings = [
        MetricReadings(name="composite", read={"a": 0.5, "b": -0.5}),
        MetricReadings(name="mcd", read={"a": -0.5, "b": -0.5}),
    ]

    report = ranking_report(judgments, readings, instrument_id="piano", outstanding=3)
