...
from optisample.carrier.store import NO_CURVE, CarrierSettings, carrier_signal, store_carrier
...
def test_a_set_carrying_no_curve_keeps_its_recording(source: Sourcer) -> None:
    ...
    assert carrier_signal(one, NO_CURVE, tempo=TEMPO) == pytest.approx(one.recording)
...
def test_dividing_by_the_written_curve_flattens_the_level(
    ...
    flattened = carrier_signal(one, envelope, tempo=TEMPO)
...
def test_the_flattened_waveform_stays_finite_under_a_curve_that_silences(
    ...
    flattened = carrier_signal(one, _written([one], settings), tempo=TEMPO)
...
def test_a_source_naming_a_loop_is_stored_around_it(source: Sourcer, carrier_settings: Settings) -> None:
    ...
    stored = store_carrier(one, NO_CURVE, settings=settings, rng=np.random.default_rng(0)).stored
...
def test_a_source_naming_no_loop_is_stored_as_the_span_it_plays(
    ...
    stored = store_carrier(source(), NO_CURVE, settings=carrier_settings(), rng=np.random.default_rng(0)).stored
...
def test_the_stored_waveform_reports_what_playback_owes_it(source: Sourcer, carrier_settings: Settings) -> None:
    ...
    carrier = store_carrier(source(peak=0.05), NO_CURVE, settings=carrier_settings(), rng=np.random.default_rng(0))
