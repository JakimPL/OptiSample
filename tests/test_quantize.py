...
def test_apply_gain_scales_linearly() -> None:
    signal = sine(440.0)
    assert np.allclose(apply_gain(signal, 0.5), 0.5 * signal)


@pytest.mark.parametrize("bits", [8, 16])
def test_undithered_snr_matches_theory(bits: int) -> None:
...
def test_undithered_output_lies_on_grid() -> None:
...
def test_dither_lowers_snr_but_stays_close_to_theory() -> None:
    signal = sine(997.0)
...
def test_requantize_clips_to_signed_range() -> None:
    step = quantization_step(8)
...
def test_noise_shaping_moves_error_toward_high_frequencies() -> None:
    signal = sine(997.0)
...
def test_requantize_default_rng_is_deterministic() -> None:
    signal = sine(440.0)
    assert np.array_equal(requantize(signal, 8), requantize(signal, 8))


def test_requantize_dither_varies_with_seed() -> None:
    signal = sine(440.0)
