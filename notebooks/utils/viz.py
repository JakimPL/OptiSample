...
*same* dynamic-range floor (``top_db`` below the peak) that the log-spectral metrics use — so what
you see faded to the floor is exactly what those metrics stop penalizing.
"""
...
from optisample.dsp.spectral import StftParams, stft_magnitude
...
def spectrogram_figure(
    signal: Signal, sample_rate: int, *, params: StftParams = StftParams(), title: str | None = None
) -> Figure:
    """Log-magnitude spectrogram on a **log-frequency** axis, in dB relative to the peak (floored ``_TOP_DB`` below).
    ...
    floor = max(peak * 10.0 ** (-_TOP_DB / 20.0), 1e-12)
    ...
    mesh = axes.pcolormesh(
        times, freqs[1:], decibels[:, 1:].T, vmin=-_TOP_DB, vmax=0.0, cmap="magma", shading="nearest"
    )
    axes.set_yscale("log")
...
def contribution_bar(comparisons: list[Row], *, title: str | None = None) -> Figure:
