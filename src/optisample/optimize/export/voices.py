...
def _slot_voices(slot: InstrumentSlot, stored: Sequence[StoredSample], sources: PlayedVoices) -> list[_Voice]:
    ...
    voices = [
        _voice(pitch, sample, played[pitch], available[pitch], _reach_s(stored[sample], pitch))
...
def _served(voice: _Voice, stored: Sequence[StoredSample], levels: Readings, sources: PlayedVoices) -> _Served:
...
def slot_members(
    ...
    played on.
    """
    voices = _slot_voices(slot, stored, sources)
    if not voices:
        return ()
